...
import {
	connect,
	disconnect,
	subscribeToNewRestaurant,
	subscribeToUpdateRestaurant,
...
export const getRestaurants = foods => {
    ...
    const params = {
      foods: foods || getState().restaurant.lastFoodQuery,
      ...getState().restaurant.dietOption,
    }
	...
	return dispatch => {
		...
		return api.saveRestaurant({ method: 'post', restaurant, user }).then(
			data => {
				...
				dispatch(
					notify(notificationTypes.SUCCESS, successMessages.ADD_RESTAURANT)
				)
			},
			err => {
	...
	return dispatch => {
		...
		return api.saveRestaurant({ method: 'put', restaurant, user }).then(
			data => {
				...
				dispatch(changeSelectedRestaurant(null))
				dispatch(
					notify(notificationTypes.SUCCESS, successMessages.UPDATE_RESTAURANT)
				)
			},
			err => {
...
export const toggleRestaurantLike = ({ restaurant, user }) => {
	...
	return dispatch => {
		...
		return api
			.manageRestaurantLikes({ restaurant, user, unlike: shouldUnlike })
			.then(null, () =>
				dispatch({
	...
	return dispatch => {
		...
		return api.submitFormToNetlify(payload).then(
			() => {
				...
				dispatch(
					notify(notificationTypes.SUCCESS, successMessages.DENUNCIATION)
				)
			},
			err => {
...
export const setDietOption = payload => {
  return dispatch => {
    dispatch({
      type: SET_DIET_OPTION,
      payload,
    })
